...
public:
    ...
    void addStudent(std::string name, std::string surname, std::string adress, int index, std::string PESEL, Sex sex);     // adds record
    void addEmployee(std::string name, std::string surname, std::string adress, float salary, std::string PESEL, Sex sex); // adds employee record
    ...
    std::shared_ptr<Person> getStudentRecord(int pos); // returns copy of record pointed by positiion in database - for testing
